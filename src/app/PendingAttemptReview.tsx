...
export function PendingAttemptReview({ review, onExit }: { review: Exclude<IeltsReview, { kind: 'objective' }>; onExit: () => void }) {
  if (review.kind === 'writing') return <PendingWritingReview submission={review.submission} onExit={onExit} backLabel={review.returnTo === 'home' ? 'Back to practice' : 'Back to results'} />
