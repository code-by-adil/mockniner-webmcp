...
type HeadingMatchingBlock = ObjectiveWebBlockRendererProps<"heading_matching_questions">["block"];
type PassageBlock = ObjectiveWebBlockRendererProps<"passage">["block"];
...
function getUsedAnswers(block: MatchingBlock, answers: Record<number, string | undefined>) {
  return new Set(
    block.questions
...
function MatchingQuestionsBlock({
  ...
  block: MatchingBlock;
  ctx: ObjectiveWebBlockRendererProps<MatchingBlockType>["ctx"];
  index: number;
}) {
...
export function HeadingMatchingPassageBlock({
  ...
  ctx: ObjectiveWebBlockRendererProps<"heading_matching_questions">["ctx"];
}) {
  ...
  return (
    <article
      ...
      {...dropFrameHandlers}
    >
      <h3 className="text-lg font-bold leading-7 text-gray-950">{passage.title}</h3>
      {passage.paragraphs.map((paragraph, paragraphIndex) => {
        ...
        const displayValue = answerValue ? optionDisplayByValue.get(answerValue) : undefined;
...
export function HeadingMatchingOptionsBlock({
  ...
  ctx,
  index,
}: ObjectiveWebBlockRendererProps<"heading_matching_questions">) {
  const groupId = getMatchingGroupId(block, index);
  const usedAnswers = getUsedAnswers(block, ctx.answers);
...
export function FeatureMatchingQuestionsBlock(
  props: ObjectiveWebBlockRendererProps<"feature_matching_questions">,
) {
  return <MatchingQuestionsBlock {...props} />;
}

export function HeadingMatchingQuestionsBlock(
  props: ObjectiveWebBlockRendererProps<"heading_matching_questions">,
) {
  return <MatchingQuestionsBlock {...props} />;
}
