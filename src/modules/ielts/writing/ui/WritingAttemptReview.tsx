...
export function WritingAttemptReview({
  ...
  const scoreData = currentPart === 1 ? evaluation.task1 : evaluation.task2;
  const { summary: evaluationSummary, overallBand, evaluatedAt, revision: evaluationRevision = 1 } = evaluation;
  const { response: essay, task, wordCount } = submittedTask;
  const isMobile = useIsMobile();
  const focusToken = focusRequest ?? selectedCorrectionId;
  const [drawer, setDrawer] = useState<{ open: boolean; request?: object | string }>({ open: true });
  ...
  const annotations = scoreData.annotations;
  const activeAnnotationIndex = annotations.findIndex(annotation => annotation.id === selectedCorrectionId);
  ...
  const focusAnnotation = useCallback((annotationIndex: number) => {
    onCorrectionSelect(annotations[annotationIndex].id);
  ...
  useEffect(() => {
    ...
    if (index < 0) return;
    const target = isMobile ? cardRefs.current[index] : annotationRefs.current[index] ?? cardRefs.current[index];
  ...
  }, [selectedCorrectionId, annotations, focusRequest, isMobile]);
      ...
      <div className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-gray-100">
          <div className="px-4 sm:px-8 pt-3 sm:pt-4 pb-0">
            <div className="flex gap-1 rounded-xl bg-gray-100/80 p-1 sm:p-1.5">
              {([1, 2] as const).map((taskId) => {
                const isActive = currentPart === taskId;
                return (
                  <button
                    key={taskId}
                    type="button"
                    onClick={() => onPartChange(taskId)}
    ...
    <div className="flex h-full flex-col bg-gray-50">
      <div className="shrink-0 border-b border-gray-200 bg-white px-4 py-3 sm:px-5">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">{issues.length} {issues.length === 1 ? "correction" : "corrections"}</span>
        </div>
      </div>
        ...
        <div className="space-y-2.5 p-3 sm:p-4 w-full">
            <section className="rounded-xl border border-emerald-200 bg-emerald-50/60 px-4 py-4">
              ...
              <dl className="mt-4 grid grid-cols-2 gap-2 text-xs">
                {[
                  [currentPart === 2 ? "Task response" : "Task achievement", scoreData.taskAchievement],
                ...
                <p className="mt-3 text-[10px] text-emerald-800/60">
                  Revision {evaluationRevision} · Saved in this browser {new Date(evaluatedAt).toLocaleString()}
                </p>
            </section>
          {issues.length === 0 ? (
