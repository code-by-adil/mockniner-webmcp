...
export function SpeakingExamRunner({ onExit, onSubmit, bindSpeakingInterview, initialPlan, onConfigurePlan, canLeave }: {
  onExit: () => void
  ...
  return <ExamUiBoundary>
    <div className="exam-live-speaking-shell flex h-screen flex-col overflow-hidden font-sans">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto pb-[max(0.5rem,env(safe-area-inset-bottom))]">
        <SpeakingInterview bindSpeakingInterview={bindSpeakingInterview} onComplete={onSubmit} initialPlan={initialPlan} onConfigurePlan={onConfigurePlan} />
      </div>
      <dialog ref={exitDialog} aria-labelledby="speaking-exit-title" className="exam-native-dialog max-w-sm rounded-xl bg-[var(--exam-surface)] p-6 text-[var(--exam-text)] shadow-xl backdrop:bg-black/40">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--exam-text-muted)]">This running interview is held in this tab. Leaving discards its recordings and progress. Your saved question set and completed attempts are kept.</p>
