...
export function createPracticeNavigation(deps: {
  ...
  return async (raw: PracticeNavigationInput, options: PracticeNavigationOptions = {}) => {
    ...
    navigating = true
    try {
      if (!options.confirmedSpeakingExit) assertCanLeave()
      if (input.action === 'library') { await deps.native.goHome(); checkCancelled(); deps.assessment.goHome(); return { view: 'home' } }
      if (input.action === 'result') {
        if (input.kind === 'assessment') await deps.assessment.openAttempt(input.attemptId, input.location && 'itemId' in input.location ? input.location.itemId : undefined, { signal, beforeOpen: deps.native.goHome })
        else await deps.native.openAttempt(input.attemptId, input.kind, input.location, { signal, beforeOpen: deps.assessment.goHome })
      ...
      if (input.kind === 'assessment') {
        ...
        deps.assessment.start(input.packageId!)
      } else {
