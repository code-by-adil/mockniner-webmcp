...
export function createPracticeTools(deps: {
  ...
  return [
    ...(['library', 'history'] as const).map(kind => ({
      ...
      description: kind === 'library' ? 'For general requests such as "let me practise Writing" or "let us do SAT", check this library first and resume suitable unfinished work or open a saved test. Author new content when the user asks for new questions. List practice IDs with duration, item/part counts, declared subject/difficulty, startability and blocking reasons, plus resumable IDs and Listening readiness. Unknown metadata is null; estimates are labeled. Availability is a snapshot, rechecked when starting. Use kind to filter and nextOffset to continue. No questions or keys.'
