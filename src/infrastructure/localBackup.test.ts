...
import { cancelBackupImport, downloadLocalBackup, prepareBackupImport, requestBackupRestore, restorePendingBackup, RESTORE_PENDING_KEY, RESTORE_NOTICE_KEY } from './localBackup';
...
describe('learner-controlled backups', () => {
  ...
  it('keeps confirmed intent and old legacy drafts on failure so a reload can retry', async () => {
    localStorage.setItem(RESTORE_PENDING_KEY, 'pending');
    localStorage.setItem('ielts-practice-session-v4', 'old draft');
    ...
    expect(localStorage.getItem('ielts-practice-session-v4')).toBe('old draft');
    expect(mocks.stage.deleteDatabaseFile).not.toHaveBeenCalled();
    await restorePendingBackup();
    expect(localStorage.getItem(RESTORE_PENDING_KEY)).toBeNull();
    expect(localStorage.getItem('ielts-practice-session-v4')).toBeNull();
